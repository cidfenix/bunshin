...
5. **Integration is configurable** (`merge.mode`). `auto` (default) = local fast-forward merge to
   ...
   `command`/`skill` shape. Absent ⇒ default, so existing repos are unchanged.
